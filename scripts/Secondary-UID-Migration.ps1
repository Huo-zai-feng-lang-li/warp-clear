# Secondary UID Migration - 二次UID迁移脚本
# 在登录新账号后执行，完成配置对象的所有权转移
# 确保所有配置正确关联到新登录的账号

Write-Host ""
Write-Host "====================================================" -ForegroundColor Cyan
Write-Host "         二次 UID 迁移 - 配置所有权转移            " -ForegroundColor Cyan
Write-Host "====================================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "执行前请确认：" -ForegroundColor Red
Write-Host "  1. 已经成功登录新的Warp账号" -ForegroundColor Yellow
Write-Host "  2. Warp已经正常运行" -ForegroundColor Yellow
Write-Host ""

$confirm = Read-Host "确认已登录新账号并继续迁移？ (y/N)"
...
# 提示是否立即启动Warp
$startWarp = Read-Host "是否立即启动Warp？ (y/N)"
if ($startWarp -eq 'y' -or $startWarp -eq 'Y') {
    Write-Log "正在启动Warp..." "Yellow"
    Start-Process "warp" -ErrorAction SilentlyContinue
    Start-Sleep -Seconds 3
    Write-Log "✓ Warp已启动，请检查MCP服务器是否正常工作" "Green"
}
